...
| 动作 | 按下 | 松开 | 行为 |
|---|---|---|---|
| 动作 1 | `0x12` (18) | `0x11` (17)→转为0x00 | 按下/未按下**双稳态**，持续上报不清零 |
| 动作 2 | `0x22` (34) | `0x21` (33)→转为0x00 | 按下/未按下**双稳态**，持续上报不清零 |
| 动作 3 | `0x32` (50) | `0x31` (49)→转为0x00 | 按下/未按下**双稳态**，持续上报不清零 |
...
- **act**: 
  - 动作 1~3：按下/未按下**双稳态**，发送后**不清零**。松开事件将 act 转为 `0x00` 并持续上报
  - 动作 4~6：单次触发，发送后自动清零
