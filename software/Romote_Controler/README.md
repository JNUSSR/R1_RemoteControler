...
| `act` | `uint8_t` | 21 | 屏幕触发的动作指令，编码 0xXY：X=动作ID(1~6)，Y=状态(1=松开,2=按下) |
...
**act（动作指令）**，编码 `0xXY`（X=动作ID, Y=状态）:

| 动作 | 按下 | 松开 | 行为 |
|---|---|---|---|
| 动作 1 | `0x12` (18) | `0x11` (17) | 按下期间**持续上报**，松开清零 |
| 动作 2 | `0x22` (34) | `0x21` (33) | 按下期间**持续上报**，松开清零 |
| 动作 3 | `0x32` (50) | `0x31` (49) | 按下期间**持续上报**，松开清零 |
| 动作 4 | `0x41` (65) | — | 单次触发，发送后立即清零 |
| 动作 5 | `0x51` (81) | — | 单次触发，发送后立即清零 |
| 动作 6 | `0x61` (97) | — | 单次触发，发送后立即清零 |
| 无动作 | `0x00` | — | — |
...
- **kfs / kfs_put**: 每次发送成功后 `memset` 清零，避免重复发送
- **act**: `MAVLink_Pack_And_Send` 内部根据动作类型智能清零：
  - 动作 1~3 按下态：**不清零**，每 50ms 持续上报直到松开事件到达
  - 动作 1~3 松开态 / 动作 4~6：发送后自动清零
